...
//
// Basic system interface for XML Service Toolkit
//
//
// IBM_PROLOG_END_TAG 
...
function xmlToJson(xml) {
	...
	if(pgmData && pgmData.length > 0) {
		for(i in pgmData) {
			...
			var dsFlag = pgmData[i].match(dsRegG);
			if(sucFlag && sucFlag.length > 0)
				rs.success = true;
			else 
				rs.success = false;
			if(dsFlag && dsFlag.length > 0)
				rs.integrity = false;
			else 
				rs.integrity = true;
			...
			if(paramData && paramData.length > 0) {
				...
				for(j in paramData) {
					...
					if(eachRow && eachRow.length > 1) 
						arr.push({"type" : eachRow[1], "value" : eachRow[2] ? eachRow[2] : ""});
				}
				rs.data = arr;
			}
			ResultArr.push(rs);
...
iConn.prototype.run = function(callback) {
...
iPgm.prototype.addReturn = function(data, type, options) {
	...
	if(options)
		this.xml += i_xml.iXmlNodeDataOpen(type, options.dim, options.varying, options.enddo, options.setlen, options.offset, options.hex, options.before, options.after, options.trim);
	else
